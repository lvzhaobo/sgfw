...
.file:hover {
	...
	textarea {width:600px;height:120px;border:2px solid #CCCCCC;color:#333333;font-size:13px;padding:2px 5px;border-radius:0.5em;}
	textarea:hover {border:2px solid #FF9900;}
	input,select {line-height:28px;width:240px;border:2px solid #CCCCCC;color:#333333;font-size:13px;font-weight:bold;padding:4px 10px;margin:0px 0px 10px 0;border-radius:0.5em;}
	input:hover,select:hover {border:2px solid #FF9900;}
	</style>
		...
		<div>
		  ...
		  <?php 
			$dir_path = "../../data/users/".$_SESSION["user"];
				...
				$result = mysql_query("select * from sgfw_homework where type='note' AND id='".$_GET["id"]."'");
				...
				$content = file_get_contents($dir_path."/".$homework["file"]);
		  ?>
		
		<div class="content" style="width:800px;float:left;margin:0 0 40px 0px;padding:20px 10px;min-height:320px;">
		  <div style="clear:float;clear:both;"></div>
		  <div style="margin:20px;">
		  <div class="note_title" style="line-height:24px;">
		    <span style="font-size:14px;font-weight:bold;"><?php echo $file_real_name;?></span><br /><span style="font-size:13px;color:#CCCCCC;"><?php echo date("Y年m月d日 H:i:s",strtotime($homework["create_time"])+6*3600)?></span>
		  </div>
		  <div class="note_content" style="line-height:24px;">
		    <?php echo join("<br />",json_decode($content));?>
		  </div>
			    <form action="../../lib/request.php?action=saveNote" method="post" id="discuss" style="display:none;">
				  <input type="text" name="title" id="note_title" placeholder="请输入笔记标题"/>
				  <?php 
				  if(array_key_exists("user",$_SESSION) && isset($_SESSION["user"]) && $_SESSION["user"]!=""){?>
				  <textarea name="content" style="" id="note_content" placeholder="请输入笔记内容"></textarea><br />
